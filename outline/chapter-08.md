# Chapter 8 — "Assembling the Assault Team" (Outline of the drafted chapter)

**Status:** Chapter fully drafted, see `chapters/chapter-08.md`. This file is a beat-by-beat reference of what's actually on the page, built for checking a NovelCrafter sync against, since NovelCrafter has repeatedly stripped sensory detail and collapsed short standalone lines into denser paragraphs on import/export. Anywhere this outline calls out a specific sensory beat or a short standalone line, that's a flag: if it's missing or flattened in a synced version, that's drift, not a deliberate edit, and needs restoring from the source file above, not treated as an acceptable variant.

## Opening — Tatsuya's own POV, sensory grounding before any dialogue

Opens on smoke and ash, not on Sevish's reaction. Specific sensory beats that need to survive any resync: the smoke smell reaching him before the words do, the taste of Shiv's ash still in his throat, his new white hair catching lamplight wrong and his brain snagging on it half a second before catching up. **Kesh** (named scout, one of the younger eastern-watch runners) reports in with blood on his sleeve that isn't his own. This grounding has to land before the report itself starts, not be summarized after.

## The scout report, textured

Kesh: raiders aren't advancing or falling back, tent's already pitched on the road's shoulder, they're just sitting, waiting. Two scouts dead, Kesh barely got back. Tatsuya's private read, in italics, one line standing alone by design: *"Waiting for what."* Then his own follow-on realization, also isolated for landing: they think they're fighting monsters, not real players, that's why they're sitting exposed instead of dug in defensive. This misread is the thing the whole ambush later exploits, needs to read as his own deduction in the moment, not narrated exposition.

## Leon's first warning, re-set from Chapter 7

Leon breaks in with the promised heads-up: monitor's acting up, a real window needed soon, framed as him following through on what he told Tatsuya he'd do. Tatsuya's private line: *"He actually did it. Said he'd warn me first, and he did."* Small beat, but it's doing real work, callback and trust-building both.

## The hillside trap, dramatized through dialogue

Correction that's load-bearing: it's Bloodhurst's own camp on the hillside, not the raiders' vantage. Only two ways down, the road (now a kill zone) or a lethal ~20m cliff. This lands through Kesh and Tatsuya's own back-and-forth ("That's not new ground. That's our own ground."), not narration explaining the geography.

## Stat/skill rundown, one connected board

His nameplate: **[LEVEL 7 – THE FIRST DESCENDANT OF SHIV]**, explicitly singular, no other goblin ever gets that exact line. Quest log: **[CALL OF THE BONES – STAGE COMPLETE: SHIV'S BLESSING]**, the level 2→7 jump explained as a quest-completion payout, with his own hedge about whether that scales like this again or was a one-off. Ten discipline points spent (4 Wisdom, 6 Fortune). Full ability list with tier/cost attached to each, not just named in a bracket: Bone Surge (no mana draw, once-daily charge), Shiv-Touched Flare (Tier 1, light mana cost), Rune Priming (Tier 3, feeds the ultimate), Call of Command (Tier 1, stamina cost, plain voice-directed orders), Gambler's Gambit (unchanged, the crit engine underneath all of it).

## Sevish proposes first, Tatsuya counters, stays in-character

Sevish's instinct: reinforce piecemeal, send goblins down in twos, hold and wear them down, framed as "clan business," and he pushes back hard on Tatsuya's standing to suggest otherwise ("You've worn Shiv's mark for an hour. Doesn't make you chieftain of this camp"). Tatsuya has to visibly recalibrate his own tone mid-scene, drop the certainty, and win the argument with an in-world tactical case, never a metagame word like "XP": easy kills make raiders bolder, not more cautious, that's the actual risk. Wyx tips the balance, not Tatsuya alone. **[AFFILIATION: THRASH CLAN 10% → 14%]** lands right after Tatsuya's actual offer to go down himself, the notification is the payoff for the roleplay, not a reward for being right.

## Leon's second warning, escalated

No longer a heads-up: "I need you now." Real urgency, distinct in weight from the first warning. Tatsuya agrees to Sevish mid-sentence, then has to circle back and confirm out loud that he meant it ("Can't eat my own words twice in one conversation").

## Gear and the two goblins sent ahead

Sevish agrees to send a small party down first to hold position without engaging: **Rott** and **Nab**, named on the page. Tatsuya gives Rott a real technique note on Goblin Gas (wait for a target that can't run) before logging off, planting the payoff for the tent ambush later.

## Real-world interlude, Sakura Arcade

- **Log-off/monitor scene:** the VRSA inspector already at the panel, tools laid out. His stream dropped off HGO's hosted slot the instant he went offline, resets to zero, doesn't just wait for him.
- **Bathroom scene, Loop virality gets real page space:** stacked notifications, the 2,140-follower jump in an hour, clips being reposted/argued over/captioned, his handle tagged without anyone knowing it's him. This needs to stay a real beat with specific clip descriptions, not a passing mention.
- **Affiliate realism:** the "Apply for Affiliate" button appearing, then the actual mechanic, 30-day minimum active-streaming history before any money, Day 1 of the clock, not instant. The retroactive gut-punch that his big stream earned nothing because he wasn't affiliate-eligible yet. Subscription price locked at ¥750/month, cosmetic-only.
- **Viewer-count realism on return:** re-login starts at **[VIEWERS: 310]**, not a snap-back to the prior peak, with his own line owning that he has to earn it back.
- **Tochi Academy notification:** real-world, not game, plain brackets. He files it away rather than engaging with it mid-crisis, a one-line internal deferral standing alone.
- **Money toward Ohema's medical costs:** a real bank transfer, **[Transfer sent: ¥15,000 → Ohema Kazehaya, Medical.]**, framed as proof-of-progress, not a promise.
- **Text to Gato:** a real exchange, not in-person, Gato's reply warm and uncomplicated, closes the thread cleanly.
- **Arena/PvP unlock realization:** the level-5 gate he crossed without noticing, **[Arena queue eligibility: unlocked at level 5.]**, filed away rather than acted on.
- **Haruto Murayama/Kansai Black Lotus beat:** a Loop headline breaking through even his own trending name, Haruto's careful public statement (never naming Eren directly), Tatsuya's private recognition of what it actually means, tied explicitly back to the original hybrid build he and Eren theorized together, which Eren only ever played the surface of. Dosuke AI speculatively tagged in the same wave of posts as a likely landing spot for Haruto.

## Arya's in-person interception

"Hey, T-Drag" from behind him in the hallway, not a call or text, catches him mid-thought on his phone. Two real declines, each explicitly tied in his own reasoning to the Eren pattern, not just plot friction (*"No. Not doing this again. Last time I let someone in this close, it cost me everything."*). She escalates into real, unperformed desperation, the guild-collapse specifics (90 down to 3, the rumor campaign after she kicked two members) and her mother's medical-school plan for her, landed as plain, unguarded dialogue, not narrated backstory. What actually moves him is the escalation, not a single line. He asks about her build, pushes her toward the advanced Hushveil upgrade as "the price of admission," ties his private reasoning (never explained to her) back to the Eren/original-build history. She commits, twenty minutes out to a different cafe with an open pod.

## Re-login and staging

Avatar picked up exactly where it was left, seated in the war tent. **[VIEWERS: 310]** stands alone as a deliberate small-number contrast to the earlier peak. Gear handed off by Sevish, twenty-minute window set, party moves out together, Rott and Nab already gone ahead.

## The assault itself (resolves on-page in this same chapter, not deferred)

- Nameplates resolve on the tent: **[RIAN] [TALO] [JOSS] [SPELLWEAVER_LYN]**, tagged **[GUILD: ANSWERS TO PK_WRAITH]**.
- Arya joins the party formally: **[ARYA – LEVEL 11 – MAGE]** (class tag deliberately not "Spellweaver," avoids colliding with the enemy Spellweaver_Lyn's name).
- Plan: Ru plus Rott and Nab take three tent walls, Arya's advanced Hushveil (Tier six, full radius) blankets the fourth/open side and the whole tent, silencing/blinding/deafening the raiders inside, who can't tell they're facing real coordination rather than monsters.
- Gas floods the tent from three sides at once, trapping them inside it this time, an explicit callback line: *"Not like last time. They're not getting outside this one."*
- Three primed runes (Rune Priming, seated ahead of time along the same three walls) trigger Ancient Bone Flame Barrage, tent collapses in fire, all four raiders die in rapid-fire kill notifications.
- The four raiders' own legacy gear still drops normally off their bodies: Ironhide Chestplate, Vanguard's Greaves, PK Raiding Crest, all sellable, courtesy of Tatsuya's high Fortune keeping it unbound. A fourth piece, Spellweaver_Lyn's own **[SHIVBONE STAFF – LEGENDARY]** (see `codex/crafting/recipes-spirit-line.md` for the item's full recipe/effect entry), drops separately and doesn't go to market, Tatsuya sets it aside on the spot for Arya, recognizing it as a real match for the mage build and the Hushveil investment he just talked her into, see `codex/characters/arya.md`.
- **Revised ending, applied:** on top of all four of those, a separate Gambler's Gambit bonus proc fires, an independent lucky roll unrelated to anything the four raiders actually carried, and pulls a single item, **[DOPPELGANGER'S BAND – UNIQUE]**, a plain iron ring with no visible enchantment. This is the concrete on-page acquisition of the signature battle item already locked in `codex/lore/battle-items-and-battlegear.md` (SP-based, no MP, no summoning, unrelated to Shiv's blessing). What it actually does isn't explained on the page, that's deliberate, left for later.
- **Chapter now ends on this reveal as the hook**, right after the explosion/kill notifications, the loot drops, and the ring's appearance, cut before the fog fully clears. Everything that was previously written past this point in the old ending (the viewer-count climb, the named chat exchange, PK_Wraith's live reaction, Arya's auction-house offer for the three sellable pieces, the walk back up the hill) has been removed from Chapter 8 and is now open material for Chapter 9's opening instead, since Chapter 9 was already slotted as the transition chapter (see `outline/chapter-outline.md`'s Chapter 8-10 section). None of that aftermath content is lost, it just hasn't been redrafted into Chapter 9 yet.

**Codex cross-references updated to match:** `codex/lore/battle-items-and-battlegear.md` and `codex/subplots/tatsuya-trap-bomber-brigand-archetype.md` no longer list the doppelganger ring as unacquired, both now point to Chapter 8 as where it lands, under the name Doppelganger's Band. `codex/characters/arya.md`, `codex/crafting/recipes-spirit-line.md`, `codex/characters/pk-wraith.md`, and `codex/subplots/world-quest-call-of-the-bones-camp-arc.md` all now note the Shivbone Staff going to Arya instead of the market.

## Craft notes specific to what NovelCrafter tends to flatten

- Several short, italicized single-line internal-monologue beats are deliberately isolated for landing (*"Waiting for what,"* *"That's not new ground. That's our own ground,"* *"He actually did it,"* etc.) — these are not the banned "isolated one-sentence narration paragraph" style rule (see `style-guide/craft-rules.md`), that rule targets prose narration, not italicized inner monologue, which has always been allowed to stand alone by house style. Don't let a resync merge these into surrounding paragraphs.
- Sensory grounding in the opening paragraph (smoke, ash-taste, hair catching light wrong) is the chapter's actual hook and has been cut or summarized in prior resyncs. It needs to survive verbatim or close to it.
- All named minor characters (Kesh, Rott, Nab, Pac_Pac, plus the confirmed handles Taja_11/AriSky) must stay named, not revert to generic role-tags.
- Bracket formatting: real-world notifications (Loop, texts, banking, Tochi) use plain, non-bold `[...]`; in-game system/HUD text uses bold `**[...]**`. This distinction has drifted in past syncs and needs checking specifically.
