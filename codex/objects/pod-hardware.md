...
- **First-gen pods (Kazuhaya Tech originals):** full-body immersion pods, the "metal coffin" style, built on Daichi's original schematics. Predate HGO. Third-gen architecture descends from first-gen DNA, which is why a manual override/hack can run HGO on one at all, but doing so causes real lag and degradation, hardware never built for the game it's now being asked to run. Leon has two of these pods, but only one is actually functional, the sole first-gen pod still running in the area, the other is a dead shell he hasn't repaired. **The degradation is ongoing, not a one-time cost:** running current the hardware was never designed to carry wears it down over real time. Sustaining an overridden pod long-term requires sourcing upgraded replacement components (the same family of parts used inside third-gen helmets) and doing hands-on manual maintenance, the same kind of upkeep Tatsuya already does on the Haya EB (see `codex/objects/haya-eb.md`). He recognizes this instinctively while performing the Chapter 2 override, reading it straight off the schematics, nobody else is going to keep this running for him. See `codex/subplots/technical-arc-leon-and-sakura-arcade.md`.
