...
- **Large level gaps can bypass normal CC resistance, new.** Power/Courage-based resistance to Stun, Root, and similar Physical-category effects (see the table above) assumes a roughly comparable fight. A combatant facing an opponent multiple real levels below them can land a hard Stun that would otherwise be resisted or reduced, the level gap itself overriding the usual resistance math, not a separate stat or item effect. This is a real, felt threat specifically in mismatched sparring or a genuinely lopsided fight, not something that comes up between characters of similar standing.
