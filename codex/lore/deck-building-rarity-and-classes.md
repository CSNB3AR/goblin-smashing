...
Separate from and unrelated to Gambler's Gambit (which belongs specifically to the Call of the Bones Necklace, see `codex/objects/call-of-the-bones-necklace.md`, and which Tatsuya doesn't have yet). Every goblin's high base Fortune feeds a passive chance, on any kill, even a trash mob, to roll a bonus item/card drop no other race gets a shot at, "Long Shot" as a working name. Most goblins barely notice it, it procs rarely. **This governs the loot drop only, not the World Quest activation itself**, see `codex/lore/world-quests.md` and `codex/subplots/world-quest-call-of-the-bones-camp-arc.md` for why "Call of the Bones" actually activates (a keyword/philosophy trigger the camp's AI recognizes, not a Fortune roll). In Chapter 3, Long Shot procs on the same kill that happens to trigger the quest, landing him the goblin gadget belt and a couple of sellable potions off two otherwise-worthless camp goblins, see `codex/objects/goblin-gadget-belt.md`.
