...
- **Chest** — Common Goblin Leather Chest, common-tier, `[+2 ENERGY]`, gifted by Wyx in Chapter 8.
...
**A character's own invested discipline points and a piece of equipment's stat bonus are always shown separately, never folded silently into one number.** Format: `[STAT: subtotal] (+X)`, where the subtotal is what the character actually built with their own points, and `(+X)` is whatever currently-equipped gear happens to be granting, removable/replaceable independent of the character's own progression. A stat with no item modifier displays with no `(+X)` at all, not a `(+0)`.

Tatsuya's Chapter 8 sheet under this convention: `[COURAGE: 4]`, `[POWER: 4]`, `[WISDOM: 8] (+5)` (necklace) totaling 13, `[SPEED: 9]` (no item modifier), `[FORTUNE: 15] (+3)` (Gambler's Earring) totaling 18, `[ENERGY: 4]`.

See `codex/lore/five-disciplines-stat-system.md` for the stat definitions this display convention applies to.
