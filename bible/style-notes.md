...
This document distills technique from a reference sample the user shared (an excerpt from *The Primal Hunter*, offered explicitly as "someone else's style," a genre-appropriate LitRPG baseline, not a template to copy). It describes what to do, not the sample's actual sentences. Nothing here quotes or closely paraphrases that text. Sits alongside Writing Bible §10, which is the non-negotiable rule set, this file is guidance for hitting that rule set well and adding what makes this book distinct.

## What the reference sample does well (worth keeping)

- **Deductive internal narration around system mechanics.** The POV character reasons through stats, screens, and unfamiliar rules out loud in his own head, testing theories, drawing conclusions, getting them half right. This already fits Tatsuya, who "thinks in numbers and odds" per Writing Bible §4, lean into it whenever he's parsing something new about HGO or the Glory Codex.
- **Inner monologue in italics AND quotation marks together** (house format, see Writing Bible §10, this has changed twice now, italics-only, then quotes-only, now both markers together as the final, standing rule), dropped directly into the narration mid-paragraph rather than set off as its own beat every time. The sample is still a useful model for how naturally it can sit inside a sentence instead of interrupting it, just apply that lesson with both markers now.
- **Dialogue paired with a tone or action beat**, never a floating line. Also already a Writing Bible rule, the sample is consistent about it, worth matching that consistency exactly.
...
- **Consistent, spare formatting for system text**, notification dings, bracketed status/skill text, kept visually distinct from prose. Compatible with the blue-box/card presentation already defined in `bible/game-system.md` §4.
